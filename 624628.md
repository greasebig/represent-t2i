...
或者只是在ddimcfgpp使用时候重写 之后还原
    ...
    e_t = model(x, timesteps[index].item() * s_in, **extra_args) 应该理解为是模型预测的噪声
...
就是需要 x_out 中 neg 的结果，     
可惜cfgdenoiser不能返回 只返回一个cfg结果       
这个怎么hook?         
因为这是函数中间的结果       
在函数前后hook都没用        
中间不知道有什么可以 hook       
...
cfg希望也能截断修改 确实可以截断 就在extra_args
...
cond=make_condition_dict(cond_in, image_cond_in)
    ...
    if shared.sd_model.model.conditioning_key == "crossattn-adm":
        ...
        if isinstance(uncond, dict):
            ...
            make_condition_dict = lambda c_crossattn, c_concat: {"c_crossattn": [c_crossattn], "c_concat": [c_concat]} 运行
...
def cfg_denoised_callback(params: CFGDenoisedParams):
    ...
    for c in ordered_callbacks('cfg_denoised'): 目前没有调用 直接返回
        try:
            c.callback(params)
        except Exception:
...
def ordered_callbacks(category, unordered_callbacks=None, *, enable_user_sort=True):
    ...
    if unordered_callbacks is None:
        unordered_callbacks = callback_map.get('callbacks_' + category, []) 进入
...
### 实现
我在文件a中有这行代码，cfg_denoised_callback(denoised_params)。我想在文件b中设置个回调，进行一些数据获取操作，该怎么实现。

class MyCallback:
    def callback(self, params: CFGDenoisedParams):
        # 在这里进行数据获取操作
        print(f"x_out: {params.x}")
        # 可以访问params中的其他属性进行更多操作
...
这种方法比之前建议的方法更加直接和简洁。它直接使用提供的on_cfg_denoised函数来注册回调，无需创建单独的回调类。
name参数是可选的，但提供一个名称可以帮助在调试或管理多个回调时更容易识别您的回调。
记住，您仍然需要确保文件b在适当的时候被导入或执行，以便注册能够生效。
...
这个函数移除当前脚本文件中注册的所有回调。它通过检查调用栈来确定当前脚本的文件名，然后移除所有与该文件名相关联的回调。
remove_callbacks_for_function(callback_func):
这个函数移除与特定回调函数相关联的所有回调。它需要您传入具体的回调函数作为参数。

考虑到您的情况，我建议使用 remove_callbacks_for_function(callback_func)，原因如下：

更精确：它只移除您特定注册的回调函数，不会影响同一文件中可能存在的其他回调。
更安全：它不依赖于调用栈检查，这在某些复杂的执行环境中可能更可靠。
更灵活：如果您的代码被其他脚本导入和使用，remove_current_script_callbacks() 可能会移除错误的回调。
...
### 采样器差异
timestep类（输入为timestep） lcm类 a1111自己实现     
euler dpm等输入为sigmas 使用kdiffusion实现    

timestep类（输入为timestep）
...
line 45, in ddim_cfgpp
        script_callbacks.on_cfg_denoised(cfg_denoised_callback, name="My CFG Denoised Callback")

    TypeError: on_cfg_denoised() got an unexpected keyword argument 'name'
...
竟然还需要kdiffusion
    ...
    in ddim_cfgpp
            noise = sigma_t * k_diffusion.sampling.torch.randn_like(x)
        NameError: name 'k_diffusion' is not defined



 noise = sigma_t * k_diffusion.sampling.torch.randn_like(x)

进入 modules/sd_samplers_common.py
...
sigma_t torch.Size([1, 1, 1, 1])
x torch.Size([1, 4, 64, 64])



last_noise_uncond 被正确赋值


/home/stable_diffusion_webui_master/modules/rng.py
...
预览都是0
    ...
    ddimcfgpp.py
    Traceback (most recent call last):
      File "/home/stable_diffusion_webui_master/modules/script_callbacks.py", line 238, in cfg_denoised_callback
        c.callback(params)
      File "/home/stable_diffusion_webui_master/extensions/ddimcfgpp-wujie/ddimcfgpp.py", line 43, in cfg_denoised_callback
        last_noise_uncond = params.x[-bs:]
