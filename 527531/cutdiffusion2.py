...
class CutDiffusionSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    ...
    def check_inputs(
        ...
        if num_images_per_prompt != 1:
            warnings.warn("num_images_per_prompt != 1 is not supported by CutDiffsion and will be ignored.")
    ...
    def __call__(
        ...
        ori_height = self.default_sample_size * self.vae_scale_factor
        ori_width = self.default_sample_size * self.vae_scale_factor
        ...
        latents_list = []
        for i in range(height//ori_height * width//ori_width):
...
def process(prompt, image_width, image_height, num_samples, seed, steps, a_prompt, n_prompt, cfg,model_name,scheduler_type):
    ...
    parser.add_argument('--experiment_name', default="HRSD")
    ...
    dpmpp_2m_karras_scheduler = DPMSolverMultistepScheduler(
        num_train_timesteps=1000,
        beta_start=0.00085,
        beta_end=0.012,
        use_karras_sigmas=True,
    ...
    # config成功赋值，但是底下没读进去？
    dpmpp_2m_karras_scheduler2 = DPMSolverMultistepScheduler.from_config(
        temp.config
    )
    ...
    # 加了temp后config里面还是没有赋值成功


    # deepcopy失败？？？ 打断点调试？？
    if scheduler_type == "original_come_from_Model":
        pipe.scheduler = global_scheduler[model_name] ## 第三次肯定进入这里了，但是为什么赋值不对？ 这个global 的值好像还是euler_a_scheduler
    elif scheduler_type == "dpmpp_2m_sde_karras_scheduler":
        pipe.scheduler = dpmpp_2m_sde_karras_scheduler
    elif scheduler_type == "euler_a_scheduler":
        pipe.scheduler = euler_a_scheduler # 这个为什么会改变全局值？
    elif scheduler_type == "dpmpp_2m_sde_karras_scheduler2":
        pipe.scheduler = dpmpp_2m_sde_karras_scheduler2 # 这个为什么会改变全局值？
    elif scheduler_type == "dpmpp_2m_karras_scheduler2":
        pipe.scheduler = dpmpp_2m_karras_scheduler2 # 这个为什么会改变全局值？    
    elif scheduler_type == "dpmpp_2m_karras_scheduler":
        pipe.scheduler = dpmpp_2m_karras_scheduler # 这个为什么会改变全局值？
    ...
    print(f"Processing prompt: {prompt},n_prompt:{n_prompt},width:{width},height:{height},multi_guidance_scale:{cfg},num_samples:{num_samples},seed:{seed}")

    try:
        # 尝试将 seed 转换为整数
        seed = int(seed)
    except :
        # 如果无法转换为整数,则将 seed 设置为 0
        seed = 0
        print(f"Invalid seed value '{seed}', setting seed to 0")
    seed = seed
    set_seed(seed)
    ...
    images = pipe(prompt, negative_prompt=n_prompt, generator=generator,
                width=width, height=height, view_batch_size=num_samples, stride=64,
                num_inference_steps=steps, 
                guidance_scale = cfg, multi_guidance_scale = cfg,
                shuffle=True, result_path=f"./output/{width}_{height}_{seed}", 
                debug=False, 
                output_type='np',
                )
