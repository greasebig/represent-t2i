...
python -m pip install --upgrade --user ortools -i https://pypi.mirrors.ustc.edu.cn/simple/
...
tb-nightly 2.17.0a20240513 requires protobuf!=4.24.0,<5.0.0,>=3.19.6, but you have protobuf 5.27.2 which is incompatible.
Successfully installed immutabledict-4.2.0 ortools-9.10.4067 protobuf-5.27.2

ImportError:
cannot import name 'CaptionProjection' from 'diffusers.models.embeddings' (/root/miniconda3/envs/forge4/lib/python3.10/site-packages/diffusers/models/embeddings.py)
...
ImportError:
cannot import name 'load_model_from_config' from 'qdiff.utils'
...
直接 conda env create -f environment.yml


UnavailableInvalidChannel: HTTP 404 NOT FOUND for channel nvidia <https://mirrors.tuna.tsinghua.edu.cn/anaconda/nvidia>


没代理也不行

pip install过的包还是import不了的问题


之后再用python -m pip install xxx安装对应包，这样保证在一个你已经确定过的python环境中，实际import也要用这个python解释器
...
},{"image_id": 579240,"id": 827600,"caption": "A large group of people seated at a long dining table with food and drinks in a room."},{"image_id": 579240,"id": 827722,"caption": "This is a large group of business professionals getting lunch."},{"image_id": 6177,"id": 827903,"caption": "A pair of scissors lies in the open position."},
...
newlytest/MixDQ/scripts/gen_calib_data.py", line 46, in main
...
修改true
    ...
    model, pipe = get_model(config.model, fp16=True, return_pipe=True, custom_pipe_cls=custom_pipe_cls)
...
但是好像需要 captions_val2014

cp utils_files/captions_val2014.json scripts/utils/
...
RuntimeError: Parent directory /share/public/diffusion_quant/calib_dataset does not exist.
...
/newlytest/MixDQ/calib_output cannot be opened.


    File "newlytest/MixDQ/scripts/gen_calib_data.py", line 128, in main
        torch.save(save_data, config.calib_data.path)
    File "/root/miniconda3/envs/myconda/lib/python3.10/site-packages/torch/serialization.py", line 628, in save
        with _open_zipfile_writer(f) as opened_zipfile:
    File "/root/miniconda3/envs/myconda/lib/python3.10/site-packages/torch/serialization.py", line 502, in _open_zipfile_writer
        return container(name_or_buffer)
    File "/root/miniconda3/envs/myconda/lib/python3.10/site-packages/torch/serialization.py", line 473, in __init__
        super().__init__(torch._C.PyTorchFileWriter(self.name))
    RuntimeError: File newlytest/MixDQ/calib_output cannot be opened.
...
但是下面的能运行成功
    ...
    def main():
        # 假设 save_data 是你要保存的数据
        save_data = {"key": "value"}

        # 假设 config.calib_data.path 是你要保存的路径
        save_path = "/newlytest/MixDQ/calib_output/test_output.pth"

        # 打印路径以确保正确
        print(f"Save path: {save_path}")
        print(f"Directory exists: {os.path.exists(os.path.dirname(save_path))}")
        print(f"Writable: {os.access(os.path.dirname(save_path), os.W_OK)}")
...
写错了
    ...
    calib_data:
        path: "newlytest/MixDQ/calib_output/bs32_t1_sdxl.pt"
...
每一步保存轨迹 turbo只有一步所以保存的很小

保存了 ts xs text_emb这些





### 第二步 Post Training Quantization (PTQ) Process

python scripts/ptq.py --config ./configs/stable-diffusion/sdxl_turbo.yaml --outdir ./logs/sdxl-turbo --seed 42



FileNotFoundError: [Errno 2] No such file or directory: './configs/stable-diffusion/sdxl-turbo.yaml'
...
calib_data:
    ...
    path: "/MixDQ/calib_output/bs32_t1_sdxl.pt"
    n_steps: 1
    batch_size: 8  # used for quant param initialization
    n_samples: 32
    scale_type: "fix"
...
之前sdxl跑的结果还有
    ...
    "traceEvents": [
    {
        "ph": "X", "cat": "cpu_op", "name": "aten::empty", "pid": 589875, "tid": 589875,
        "ts": 1718273742688378, "dur": 19,
        "args": {
        "External id": 1,"Ev Idx": 0
        }
    },
    {
        "ph": "X", "cat": "cpu_op", "name": "aten::to", "pid": 589875, "tid": 589875,
        "ts": 1718273742688418, "dur": 1,
        "args": {
        "External id": 2,"Ev Idx": 1
        }
    },
    {
        "ph": "X", "cat": "cpu_op", "name": "aten::lift_fresh", "pid": 589875, "tid": 589875,
        "ts": 1718273742688428, "dur": 1,
        "args": {
        "External id": 3,"Ev Idx": 2
        }
    },
    {
        "ph": "X", "cat": "cpu_op", "name": "aten::detach_", "pid": 589875, "tid": 589875,
        "ts": 1718273742688432, "dur": 9,
        "args": {
        "External id": 4,"Ev Idx": 3
        }
    },
    {
        "ph": "X", "cat": "cpu_op", "name": "detach_", "pid": 589875, "tid": 589875,
        "ts": 1718273742688436, "dur": 4,
        "args": {
        "External id": 5,"Ev Idx": 4
        }
    },

不知啥玩意

python scripts/ptq.py --config ./configs/stable-diffusion/${cfg_name} --outdir ./logs/$1 --seed 42


假设你的脚本名为 run_model.sh，它的内容是你提供的代码，那么运行这个脚本时，你可以这样输入命令：

    bash
    复制代码
    ./run_model.sh output_directory 0
在这个命令中：

output_directory 是第一个参数，对应 $1。它将替换 --outdir ./logs/$1 中的 $1，表示日志的输出目录。
0 是第二个参数，对应 $2。它将替换 CUDA_VISIBLE_DEVICES=$2 中的 $2，表示选择的 CUDA 设备编号。


main_ptq.sh $LOG_NAME $GPU_ID
...
newlytest/MixDQ/scripts/ptq.py", line 127, in main
    _ = qnn(calib_xs[:calib_batch_size].cuda(), calib_ts[:calib_batch_size].cuda(), calib_cs[:calib_batch_size].cuda(), added_cond_kwargs=calib_added_conds)

MixDQ/quant_utils/qdiff/models/quant_layer.py", line 98, in forward
...
要将这些数据转换为半精度浮点数（fp16），可以使用 PyTorch 的 half() 方法。以下是转换这些数据并传递给 qnn 函数的代码：

    python
    复制代码
    calib_xs_fp16 = calib_xs[:calib_batch_size].cuda().half()
    calib_ts_fp16 = calib_ts[:calib_batch_size].cuda().half()
    calib_cs_fp16 = calib_cs[:calib_batch_size].cuda().half()

    _ = qnn(calib_xs_fp16, calib_ts_fp16, calib_cs_fp16, added_cond_kwargs=calib_added_conds)
这样，calib_xs、calib_ts 和 calib_cs 这三个数据集的子集会被转换为半精度浮点数，然后再传递给 qnn 函数。


    calib_xs = calib_xs.half()
    calib_ts = calib_ts.half()
    calib_cs = calib_cs.half()
...
终于可以了
    ...
    06/27/2024 10:19:56 - INFO - __main__ -   Not use gradient checkpointing for transformer blocks
    06/27/2024 10:19:56 - INFO - __main__ -   Sampling data from 1 timesteps for calibration
    06/27/2024 10:19:56 - INFO - qdiff.utils -   Selected 1 steps from 1 sampling steps
    06/27/2024 10:19:57 - INFO - __main__ -   Calibration data shape: torch.Size([32, 4, 64, 64]) torch.Size([32]) torch.Size([32, 77, 2048]) torch.Size([32, 1280]) torch.Size([32, 6])
    06/27/2024 10:20:00 - INFO - __main__ -   weight initialization done!
    06/27/2024 10:20:00 - INFO - __main__ -   Running stat for activation quantization
    100%|███████████████████████████| 4/4 [00:10<00:00,  2.69s/it]
    06/27/2024 10:20:11 - INFO - __main__ -   activation initialization done!
    06/27/2024 10:20:11 - INFO - __main__ -   Saving calibrated quantized UNet model
...
![alt text](assets/6367/image-22.png)
...
python scripts/quant_txt2img.py --base_path $CKPT_PATH --batch_size 2 --num_imgs 8 



Loading quantized model checkpoint
06/27/2024 10:35:13 - INFO - __main__ -   Inference without mixed precision!
06/27/2024 10:35:13 - INFO - __main__ -   starting from image 0
100%|███████████| 1/1 [00:03<00:00,  3.29s/it]
100%|███████████| 1/1 [00:03<00:00,  3.44s/it]
100%|███████████| 1/1 [00:03<00:00,  3.52s/it]
100%|███████████| 1/1 [00:03<00:00,  3.39s/it]
Generating image samples for FID evaluation.: 


基本流程是 先加载原底模 在加载量化模型 load参数       
...
[Submitted on 8 Feb 2023 (v1), last revised 8 Jun 2023 (this version, v3)]


扩散模型通过使用深度神经网络进行迭代噪声估计，在图像合成中取得了巨大成功。然而，噪声估计模型的推理速度慢、内存消耗高、计算强度大，阻碍了扩散模型的有效采用。虽然训练后量化 (PTQ) 被认为是其他任务的首选压缩方法，但它在扩散模型上并不是开箱即用的。我们提出了一种新颖的 PTQ 方法，专门针对扩散模型独特的多时间步长管道和模型架构，该方法压缩噪声估计网络以加速生成过程。我们认为扩散模型量化的关键难点是噪声估计网络在多个时间步长上不断变化的输出分布以及噪声估计网络内快捷层的双峰激活分布。在这项工作中，我们通过时间步长感知校准和分割快捷量化来应对这些挑战。实验结果表明，我们提出的方法能够将全精度无条件扩散模型量化为 4 位，同时以无需训练的方式保持相当的性能（FID 变化很小，最多为 2.34，而传统 PTQ 则 >100）。我们的方法还可以应用于文本引导的图像生成，我们首次可以在 4 位权重中以高生成质量运行稳定的扩散。


[NEW!] Q-Diffusion is featured by NVIDIA TensorRT! Check out the official example.

![alt text](assets/6367/image-20.png)


This repository provides the official implementation for Q-Diffusion with calibrated (simulated) quantized checkpoints.


![alt text](assets/6367/image-21.png)

用法

对于潜在扩散和稳定扩散实验，首先按照 CompVis 中潜在扩散和稳定扩散存储库中的说明下载相关检查点。我们目前用于sd-v1-4.ckpt稳定扩散。

从 Google Drive [链接] 下载量化检查点。使用 4/8 位权重量化的检查点与使用 4/8 位权重和 8 位激活量化的检查点相同。

然后使用以下命令运行带有量化检查点的推理脚本：
...
要进行校准过程，您必须首先生成相应的校准数据集。我们在此处提供了一些示例校准数据集。这些数据集在每个时间步骤包含大约 1000-2000 个中间输出样本，这对于校准目的来说已经足够了。我们很快将上传满足校准最低要求的较小子集。与此同时，您可以考虑按照本文中描述的步骤自行生成校准数据集。
...
我们的代码是基于ddim、latent-diffusion和stable-diffusion开发的。我们参考了BRECQ进行分块校准实现。

我们感谢DeepSpeed提供的模型尺寸和 BOPS 测量以及用于 IS 和 FID 计算的火炬保真度。

We thank DeepSpeed for model sizes and BOPS measurement and torch-fidelity for IS and FID computation.
